...
@section('content')
    ...
    <div class="container">
        ...
        <div class="row ">
            <a href="{{ route('admin.banner') }} ">gestion des bannière <img class="col s12" src="{{asset('asset/banner/acceuil.png')}}" alt=""></a>
        </div>

        <div class="row ">
            <form action="{{ route('admin.product') }}" method="POST">
                @csrf
                <label for="search_ean">Recherchez un produit:</label>
                <input required pattern = "[0-9]{13}" maxlength = '13' type="search" id="search_ean" name="search_ean" value="<?php if(isset($_POST['search_ean'])){
                echo $_POST['search_ean'];}?>">
